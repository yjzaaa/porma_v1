/**
 * 【第 7 层 - ASR Provider 层】豆包 ASR Provider
 *
 * 这是豆包 ASR 的门面层，只保留对外协议和整体生命周期。
 *
 * 拆分后的内部层次：
 * - audio/：PCM 帧积压、降采样、分片发送
 * - transport/：主进程 start / state / transcript / buffer 桥接
 * - provider 本身：会话状态、停止等待、资源清理
 *
 * 数据流：
 *   Session PCM 帧 → audio 层 → transport 层 → 主进程
 *   → 豆包 ASR → transport 层 → provider 状态 → 渲染进程
 *
 * 特性：
 *   - 回取免提缓冲（获取 VAD 触发前的音频上下文）
 *   - PCM 实时分片（200ms 块）流式发送
 *   - 事件监听隔离（sessionId 守卫，防止跨会话串扰）
 *
 * @see ../../../shared/utils/pcm.ts - PCM 工具函数
 * @see ../../../shared/types/asr.ts - ASRProvider 接口定义
 * @see ../core/modules/VoiceCaptureModule.ts - 录音会话与 PCM 来源
 */

import type { ASRProvider, ASREventListener } from '../../../shared/types/asr'
import { createASREventBus, subscribeASREvents } from '../../../shared/types/asr'
import type { PcmFrame } from '../../../shared/types/panel'
import type { VoiceAsrTransportBus } from '../../../shared/bus/VoiceAsrTransportBus'
import { DoubaoAudioPipeline } from './audio'
import { DoubaoTransportBridge } from './transport'

export class DoubaoProvider implements ASRProvider {
  /** ASR 事件总线 */
  private readonly eventBus = createASREventBus()
  /** 外部交互总线 */
  private readonly transport: VoiceAsrTransportBus
  /** 音频缓冲与分片层 */
  private readonly audio: DoubaoAudioPipeline
  /** 主进程桥接层 */
  private readonly transportBridge: DoubaoTransportBridge
  /** 当前 ASR 会话 ID（用于 IPC 消息隔离） */
  private sessionId: string | null = null
  /** 主进程 ASR 是否就绪（startVoiceDictation 成功返回后为 true） */
  private asrReady = false
  /** 当前definite状态 */
  private currentDefinite?: boolean
  /** utterances信息 */
  private currentUtterances?: Array<{text: string, definite: boolean}>
  /** 是否正在停止中（阻止新数据处理） */
  private stopping = false
  /** 主进程返回的累积转写文本 */
  private transcriptText = ''
  /** 停止等待 Promise 的 resolve（等待主进程 session 真正结束） */
  private stopWaitResolve: (() => void) | null = null
  /** 停止等待 Promise 的兜底超时 */
  private stopWaitTimeout: ReturnType<typeof setTimeout> | null = null
  /** IPC 监听器清理函数 */
  private cleanupListeners: (() => void) | null = null

  constructor(transport?: VoiceAsrTransportBus) {
    if (!transport) {
      throw new Error('DoubaoProvider 需要 ASR 对外交互总线')
    }
    this.transport = transport
    this.audio = new DoubaoAudioPipeline({
      getSessionId: () => this.sessionId,
      isStopping: () => this.stopping,
      isAsrReady: () => this.asrReady,
      sendAudioChunk: (chunk: ArrayBuffer) => this.sendAudioChunk(chunk),
    })
    this.transportBridge = new DoubaoTransportBridge({
      eventBus: this.eventBus,
      transport: this.transport,
      getSessionId: () => this.sessionId,
      isStopping: () => this.stopping,
      setAsrReady: (ready: boolean) => {
        this.asrReady = ready
      },
      flushQueuedAudio: () => {
        this.audio.flushQueuedAudio()
      },
      resolveStopWait: () => {
        this.resolveStopWait()
      },
      setCurrentDefinite: (value: boolean | undefined) => {
        this.currentDefinite = value
      },
      setCurrentUtterances: (value: Array<{ text: string; definite: boolean }> | undefined) => {
        this.currentUtterances = value
      },
      setTranscriptText: (value: string) => {
        this.transcriptText = value
      },
      sendAudioChunk: (chunk: ArrayBuffer) => {
        this.sendAudioChunk(chunk)
      },
    })
  }

  onEvent(listener: ASREventListener): () => void {
    return subscribeASREvents(this.eventBus, listener)
  }

  /**
   * 启动豆包 ASR 会话
   */
  async start(): Promise<void> {
    const sid = crypto.randomUUID()
    this.prepareStartSession(sid)
    await this.startTransportSession(sid)
  }

  /**
   * 接收会话侧推送的 PCM 帧并转发给主进程
   */
  pushAudio(frame: PcmFrame): void {
    this.audio.pushAudio(frame)
  }

  /**
   * 主动停止 ASR 并等待最终结果
   *
   * @returns 主进程返回的最终转写文本
   *
   * 清理顺序：停止音频采集 → IPC 通知主进程停止 ASR → 清空待发送缓冲
   */
  async stop(): Promise<string> {
    this.stopping = true
    this.asrReady = false
    const sid = this.sessionId

    // 通知主进程停止 ASR，并等待最终 transcript/idle 状态回流，避免尾字丢失
    if (sid) {
      const waitDone = this.waitForStopCompletion()
      await this.transport.request('stopVoiceDictation', { sessionId: sid }).catch(() => {})
      await waitDone
    }
    this.audio.clear()

    return this.transcriptText.trim()
  }

  /**
   * 取消 ASR（丢弃结果）
   *
   * 不清除 transcriptText（但外部调用后也不会用它）。
   */
  async cancel(): Promise<void> {
    this.stopping = true
    this.asrReady = false
    const sid = this.sessionId
    if (sid) { this.transport.request('cancelVoiceDictation', { sessionId: sid }).catch(() => {}) }
    this.resolveStopWait()
    this.audio.clear()
  }

  /**
   * 获取当前识别的详细信息
   */
  getCurrentRecognitionDetails(): {
    text: string
    isFinal: boolean
    confidence: number
    definite?: boolean
    utterances?: Array<{text: string, definite: boolean}>
  } {
    return {
      text: this.transcriptText,
      isFinal: false, // 豆包ASR在onend时才确定最终
      confidence: 0.8, // 置信度可以取自result.confidence
      definite: this.currentDefinite,
      utterances: this.currentUtterances
    }
  }

  /** 释放所有资源（幂等） */
  dispose(): void {
    this.cleanupListeners?.()
    this.cleanupListeners = null
    if (this.stopWaitTimeout) clearTimeout(this.stopWaitTimeout)
    this.stopWaitTimeout = null
    this.stopWaitResolve = null
    this.cancel().catch(() => {})
    this.sessionId = null
    this.asrReady = false
    this.currentDefinite = undefined
    this.currentUtterances = undefined
    this.eventBus.clear()
  }

  /**
   * 发送单个 PCM 分片。
   */
  private sendAudioChunk(chunk: ArrayBuffer): void {
    const sid = this.sessionId
    if (!sid || this.stopping) return
    this.transport.request('sendVoiceDictationAudio', { sessionId: sid, data: chunk }).catch(() => {})
  }

  /**
   * 等待主进程关闭 ASR 会话，确保最终 transcript 已回流。
   */
  private waitForStopCompletion(): Promise<void> {
    this.resolveStopWait()
    return new Promise<void>((resolve) => {
      this.stopWaitResolve = resolve
      this.stopWaitTimeout = setTimeout(() => {
        this.resolveStopWait()
      }, 1500)
    })
  }

  /**
   * 结束 stop 等待（幂等）。
   */
  private resolveStopWait(): void {
    if (this.stopWaitTimeout) {
      clearTimeout(this.stopWaitTimeout)
      this.stopWaitTimeout = null
    }
    const resolve = this.stopWaitResolve
    this.stopWaitResolve = null
    resolve?.()
  }

  /**
   * 准备会话启动前的本地状态与监听器。
   */
  private prepareStartSession(sessionId: string): void {
    this.resetSessionState(sessionId)
    this.cleanupListeners = this.transportBridge.registerListeners()
  }

  /**
   * 重置本次会话的本地状态。
   */
  private resetSessionState(sessionId: string): void {
    this.cleanupListeners?.()
    this.cleanupListeners = null
    this.stopping = false
    this.asrReady = false
    this.transcriptText = ''
    this.sessionId = sessionId
    this.currentDefinite = undefined
    this.currentUtterances = undefined
  }

  /**
   * 启动主进程 ASR，会话就绪后回放免提缓冲。
   */
  private async startTransportSession(sessionId: string): Promise<void> {
    await this.transportBridge.startTransportSession(sessionId)
  }
}
